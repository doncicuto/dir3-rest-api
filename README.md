...
Actualmente estoy usando Drone para crear una imagen Drone y desplegarse en K8s.

El hecho es que las imágenes son creadas pero a veces el despliegue no realiza el rollout, así que de momento se hace un rollout manual con el siguiente comando:

`kubectl rollout restart deployment/dir3-rest-api-deployment -n dir3`

Intentemos forzar el rollout desde Drone We'll try to force this rollup from drone as soon as we found a clean and simple way.
...
Currently I'm using Drone to create the Docker image and deploy it to K8s.

The fact is that images are created but sometimes the deployment does not rollout, so for now a manual K8s rollout is made with the following command:

`kubectl rollout restart deployment/dir3-rest-api-deployment -n dir3`

We'll try to force this rollup from drone as soon as we found a clean and simple way.
